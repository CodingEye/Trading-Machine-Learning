...
CCatBoost::~CCatBoost(void)
 {
   ...
   FileFlush((int)onnx_handle);
 }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
bool CCatBoost::OnnxLoad(long &handle)
 {
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CCatBoost::Init(string onnx_filename, uint flags=ONNX_DEFAULT)
 {       
...
//|                                                                  |
//+------------------------------------------------------------------+
bool CCatBoost::Init(const uchar &onnx_buff[], ulong flags=ONNX_DEFAULT)
 {  
...
void CCatBoost::PrintTypeInfo(const long num,const string layer,const OnnxTypeInfo& type_info)
  {
...
//|                                                                  |
//+------------------------------------------------------------------+
vector CCatBoost::predict_proba(const vector &x)
 {
   vector proba = {};
   if (!this.initialized)
    {
...
//---
   ...
   if (!OnnxRun(onnx_handle, ONNX_DATA_TYPE_FLOAT, x_float, output_data, output_data_map))
     ...
     proba = ArrayToVector(output_data_map[0].value);
...
//|                                                                  |
//+------------------------------------------------------------------+
matrix CCatBoost::predict_proba(const matrix &x)
 {
   matrix preds = {};
...
//|                                                                  |
//+------------------------------------------------------------------+
vector CCatBoost::predict_bin(const matrix &x)
 {
   vector preds(x.Rows());
   for (ulong i=0; i<x.Rows(); i++)
     {
...
//|                                                                  |
//+------------------------------------------------------------------+
long CCatBoost::predict_bin(const vector &x)
 {
   if (!this.initialized)
    {
      printf("%s The model is not initialized yet to make predictions | call Init function first",__FUNCTION__);
